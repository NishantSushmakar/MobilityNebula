...
OveraboveTspatialStboxLogicalFunction::OveraboveTspatialStboxLogicalFunction(
...
LogicalFunction OveraboveTspatialStboxLogicalFunction::withDataType(const DataType& dataType) const {
    auto copy = *this;
...
LogicalFunction OveraboveTspatialStboxLogicalFunction::withChildren(const std::vector<LogicalFunction>& children) const {
    PRECONDITION(children.size() == 2, "OveraboveTspatialStboxLogicalFunction requires 2 children, but got {}", children.size());
    auto copy = *this;
...
std::string OveraboveTspatialStboxLogicalFunction::explain(ExplainVerbosity verbosity) const {
    ...
    return fmt::format("OVERABOVE_TSPATIAL_STBOX({})", args);
}
...
LogicalFunctionRegistryReturnType LogicalFunctionGeneratedRegistrar::RegisterOveraboveTspatialStboxLogicalFunction(LogicalFunctionRegistryArguments arguments) {
    PRECONDITION(arguments.children.size() == 2, "OveraboveTspatialStboxLogicalFunction requires 2 children, but got {}", arguments.children.size());
    return OveraboveTspatialStboxLogicalFunction(arguments.children[0], arguments.children[1]);
}

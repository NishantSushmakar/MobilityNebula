...
EcoversTgeoGeoLogicalFunction::EcoversTgeoGeoLogicalFunction(
...
LogicalFunction EcoversTgeoGeoLogicalFunction::withDataType(const DataType& dataType) const {
    auto copy = *this;
...
LogicalFunction EcoversTgeoGeoLogicalFunction::withChildren(const std::vector<LogicalFunction>& children) const {
    PRECONDITION(children.size() == 2, "EcoversTgeoGeoLogicalFunction requires 2 children, but got {}", children.size());
    auto copy = *this;
...
std::string EcoversTgeoGeoLogicalFunction::explain(ExplainVerbosity verbosity) const {
    ...
    return fmt::format("ECOVERS_TGEO_GEO({})", args);
}
...
LogicalFunctionRegistryReturnType LogicalFunctionGeneratedRegistrar::RegisterEcoversTgeoGeoLogicalFunction(LogicalFunctionRegistryArguments arguments) {
    PRECONDITION(arguments.children.size() == 2, "EcoversTgeoGeoLogicalFunction requires 2 children, but got {}", arguments.children.size());
    return EcoversTgeoGeoLogicalFunction(arguments.children[0], arguments.children[1]);
}

...
LogicalFunction EdwithinTgeoGeoLogicalFunction::withDataType(const DataType& dataType) const {
    auto copy = *this;
...
LogicalFunction EdwithinTgeoGeoLogicalFunction::withChildren(const std::vector<LogicalFunction>& children) const {
    PRECONDITION(children.size() == 3, "EdwithinTgeoGeoLogicalFunction requires 3 children, but got {}", children.size());
    auto copy = *this;
...
std::string EdwithinTgeoGeoLogicalFunction::explain(ExplainVerbosity verbosity) const {
    ...
    return fmt::format("EDWITHIN_TGEO_GEO({})", args);
}
...
LogicalFunctionRegistryReturnType LogicalFunctionGeneratedRegistrar::RegisterEdwithinTgeoGeoLogicalFunction(LogicalFunctionRegistryArguments arguments) {
    PRECONDITION(arguments.children.size() == 3, "EdwithinTgeoGeoLogicalFunction requires 3 children, but got {}", arguments.children.size());

...
RightStboxTspatialLogicalFunction::RightStboxTspatialLogicalFunction(
...
LogicalFunction RightStboxTspatialLogicalFunction::withDataType(const DataType& dataType) const {
    auto copy = *this;
...
LogicalFunction RightStboxTspatialLogicalFunction::withChildren(const std::vector<LogicalFunction>& children) const {
    PRECONDITION(children.size() == 2, "RightStboxTspatialLogicalFunction requires 2 children, but got {}", children.size());
    auto copy = *this;
...
std::string RightStboxTspatialLogicalFunction::explain(ExplainVerbosity verbosity) const {
    ...
    return fmt::format("RIGHT_STBOX_TSPATIAL({})", args);
}
...
LogicalFunctionRegistryReturnType LogicalFunctionGeneratedRegistrar::RegisterRightStboxTspatialLogicalFunction(LogicalFunctionRegistryArguments arguments) {
    PRECONDITION(arguments.children.size() == 2, "RightStboxTspatialLogicalFunction requires 2 children, but got {}", arguments.children.size());
    return RightStboxTspatialLogicalFunction(arguments.children[0], arguments.children[1]);
}

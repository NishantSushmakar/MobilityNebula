...
AboveTspatialTspatialLogicalFunction::AboveTspatialTspatialLogicalFunction(
...
LogicalFunction AboveTspatialTspatialLogicalFunction::withDataType(const DataType& dataType) const {
    auto copy = *this;
...
LogicalFunction AboveTspatialTspatialLogicalFunction::withChildren(const std::vector<LogicalFunction>& children) const {
    PRECONDITION(children.size() == 2, "AboveTspatialTspatialLogicalFunction requires 2 children, but got {}", children.size());
    auto copy = *this;
...
std::string AboveTspatialTspatialLogicalFunction::explain(ExplainVerbosity verbosity) const {
    ...
    return fmt::format("ABOVE_TSPATIAL_TSPATIAL({})", args);
}
...
LogicalFunctionRegistryReturnType LogicalFunctionGeneratedRegistrar::RegisterAboveTspatialTspatialLogicalFunction(LogicalFunctionRegistryArguments arguments) {
    PRECONDITION(arguments.children.size() == 2, "AboveTspatialTspatialLogicalFunction requires 2 children, but got {}", arguments.children.size());
    return AboveTspatialTspatialLogicalFunction(arguments.children[0], arguments.children[1]);
}

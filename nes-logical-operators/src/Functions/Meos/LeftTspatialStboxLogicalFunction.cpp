...
LeftTspatialStboxLogicalFunction::LeftTspatialStboxLogicalFunction(
...
LogicalFunction LeftTspatialStboxLogicalFunction::withDataType(const DataType& dataType) const {
    auto copy = *this;
...
LogicalFunction LeftTspatialStboxLogicalFunction::withChildren(const std::vector<LogicalFunction>& children) const {
    PRECONDITION(children.size() == 2, "LeftTspatialStboxLogicalFunction requires 2 children, but got {}", children.size());
    auto copy = *this;
...
std::string LeftTspatialStboxLogicalFunction::explain(ExplainVerbosity verbosity) const {
    ...
    return fmt::format("LEFT_TSPATIAL_STBOX({})", args);
}
...
LogicalFunctionRegistryReturnType LogicalFunctionGeneratedRegistrar::RegisterLeftTspatialStboxLogicalFunction(LogicalFunctionRegistryArguments arguments) {
    PRECONDITION(arguments.children.size() == 2, "LeftTspatialStboxLogicalFunction requires 2 children, but got {}", arguments.children.size());
    return LeftTspatialStboxLogicalFunction(arguments.children[0], arguments.children[1]);
}

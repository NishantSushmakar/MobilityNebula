...
EverNeTgeoGeoLogicalFunction::EverNeTgeoGeoLogicalFunction(
...
LogicalFunction EverNeTgeoGeoLogicalFunction::withDataType(const DataType& dataType) const {
    auto copy = *this;
...
LogicalFunction EverNeTgeoGeoLogicalFunction::withChildren(const std::vector<LogicalFunction>& children) const {
    PRECONDITION(children.size() == 2, "EverNeTgeoGeoLogicalFunction requires 2 children, but got {}", children.size());
    auto copy = *this;
...
std::string EverNeTgeoGeoLogicalFunction::explain(ExplainVerbosity verbosity) const {
    ...
    return fmt::format("EVER_NE_TGEO_GEO({})", args);
}
...
LogicalFunctionRegistryReturnType LogicalFunctionGeneratedRegistrar::RegisterEverNeTgeoGeoLogicalFunction(LogicalFunctionRegistryArguments arguments) {
    PRECONDITION(arguments.children.size() == 2, "EverNeTgeoGeoLogicalFunction requires 2 children, but got {}", arguments.children.size());
    return EverNeTgeoGeoLogicalFunction(arguments.children[0], arguments.children[1]);
}

...
EdwithinTgeoTgeoLogicalFunction::EdwithinTgeoTgeoLogicalFunction(
...
LogicalFunction EdwithinTgeoTgeoLogicalFunction::withDataType(const DataType& dataType) const {
    auto copy = *this;
...
LogicalFunction EdwithinTgeoTgeoLogicalFunction::withChildren(const std::vector<LogicalFunction>& children) const {
    PRECONDITION(children.size() == 3, "EdwithinTgeoTgeoLogicalFunction requires 3 children, but got {}", children.size());
    auto copy = *this;
...
std::string EdwithinTgeoTgeoLogicalFunction::explain(ExplainVerbosity verbosity) const {
    ...
    return fmt::format("EDWITHIN_TGEO_TGEO({})", args);
}
...
LogicalFunction EdwithinTgeoTgeoLogicalFunction::withInferredDataType(const Schema& schema) const {
    ...
    INVARIANT(newChildren[2].getDataType().isNumeric(), "param2 must be numeric, but was: {}", newChildren[2].getDataType());
...
LogicalFunctionRegistryReturnType LogicalFunctionGeneratedRegistrar::RegisterEdwithinTgeoTgeoLogicalFunction(LogicalFunctionRegistryArguments arguments) {
    PRECONDITION(arguments.children.size() == 3, "EdwithinTgeoTgeoLogicalFunction requires 3 children, but got {}", arguments.children.size());
    return EdwithinTgeoTgeoLogicalFunction(arguments.children[0], arguments.children[1], arguments.children[2]);
}

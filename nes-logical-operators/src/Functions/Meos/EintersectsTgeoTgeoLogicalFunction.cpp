...
EintersectsTgeoTgeoLogicalFunction::EintersectsTgeoTgeoLogicalFunction(
...
LogicalFunction EintersectsTgeoTgeoLogicalFunction::withDataType(const DataType& dataType) const {
    auto copy = *this;
...
LogicalFunction EintersectsTgeoTgeoLogicalFunction::withChildren(const std::vector<LogicalFunction>& children) const {
    PRECONDITION(children.size() == 2, "EintersectsTgeoTgeoLogicalFunction requires 2 children, but got {}", children.size());
    auto copy = *this;
...
std::string EintersectsTgeoTgeoLogicalFunction::explain(ExplainVerbosity verbosity) const {
    ...
    return fmt::format("EINTERSECTS_TGEO_TGEO({})", args);
}
...
LogicalFunctionRegistryReturnType LogicalFunctionGeneratedRegistrar::RegisterEintersectsTgeoTgeoLogicalFunction(LogicalFunctionRegistryArguments arguments) {
    PRECONDITION(arguments.children.size() == 2, "EintersectsTgeoTgeoLogicalFunction requires 2 children, but got {}", arguments.children.size());
    return EintersectsTgeoTgeoLogicalFunction(arguments.children[0], arguments.children[1]);
}

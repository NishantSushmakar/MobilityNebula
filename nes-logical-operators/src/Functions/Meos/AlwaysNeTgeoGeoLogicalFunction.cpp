...
AlwaysNeTgeoGeoLogicalFunction::AlwaysNeTgeoGeoLogicalFunction(
...
LogicalFunction AlwaysNeTgeoGeoLogicalFunction::withDataType(const DataType& dataType) const {
    auto copy = *this;
...
LogicalFunction AlwaysNeTgeoGeoLogicalFunction::withChildren(const std::vector<LogicalFunction>& children) const {
    PRECONDITION(children.size() == 2, "AlwaysNeTgeoGeoLogicalFunction requires 2 children, but got {}", children.size());
    auto copy = *this;
...
std::string AlwaysNeTgeoGeoLogicalFunction::explain(ExplainVerbosity verbosity) const {
    ...
    return fmt::format("ALWAYS_NE_TGEO_GEO({})", args);
}
...
LogicalFunctionRegistryReturnType LogicalFunctionGeneratedRegistrar::RegisterAlwaysNeTgeoGeoLogicalFunction(LogicalFunctionRegistryArguments arguments) {
    PRECONDITION(arguments.children.size() == 2, "AlwaysNeTgeoGeoLogicalFunction requires 2 children, but got {}", arguments.children.size());
    return AlwaysNeTgeoGeoLogicalFunction(arguments.children[0], arguments.children[1]);
}

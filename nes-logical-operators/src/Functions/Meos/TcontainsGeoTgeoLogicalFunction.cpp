...
TcontainsGeoTgeoLogicalFunction::TcontainsGeoTgeoLogicalFunction(LogicalFunction lon1, LogicalFunction lat1, LogicalFunction timestamp1, LogicalFunction staticGeometry)
...
TcontainsGeoTgeoLogicalFunction::TcontainsGeoTgeoLogicalFunction(LogicalFunction lon1, LogicalFunction lat1, LogicalFunction timestamp1, LogicalFunction lon2, LogicalFunction lat2, LogicalFunction timestamp2)
    : dataType(DataTypeProvider::provideDataType(DataType::Type::INT32))
...
{
    PRECONDITION(children.size() == 4 || children.size() == 6, "TcontainsGeoTgeoLogicalFunction requires 4 or 6 children, but got {}", children.size());
    auto copy = *this;
...
std::string TcontainsGeoTgeoLogicalFunction::explain(ExplainVerbosity verbosity) const
{
    ...
    return fmt::format("TEMPORAL_TCONTAINS_GEO_TGEO_GEOMETRY({})", args);
}
...
LogicalFunctionRegistryReturnType LogicalFunctionGeneratedRegistrar::RegisterTcontainsGeoTgeoLogicalFunction(LogicalFunctionRegistryArguments arguments)
{
    if (arguments.children.size() == 4) {
        ...
        return TcontainsGeoTgeoLogicalFunction(arguments.children[0], arguments.children[1], arguments.children[2], arguments.children[3], arguments.children[4], arguments.children[5]);
    } else {
        PRECONDITION(false, "TcontainsGeoTgeoLogicalFunction requires 4 or 6 children, but got {}", arguments.children.size());
    }
}

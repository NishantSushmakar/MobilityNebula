...
OverbeforeStboxTspatialPhysicalFunction::OverbeforeStboxTspatialPhysicalFunction(        PhysicalFunction param0Function,
    ...
    const auto result = nautilus::invoke(
        +[](const char* param0_ptr, uint32_t param0_size, const char* param1_ptr, uint32_t param1_size) -> int32_t {
            try {
                ...
                MEOS::STBox* param0_obj = MEOS::stbox_in(param0_str.c_str());
                if (!param0_obj) {
                ...
                auto meos_result = MEOS::overbefore_stbox_tspatial(param0_obj, param1_obj);
...
PhysicalFunctionGeneratedRegistrar::RegisterOverbeforeStboxTspatialPhysicalFunction(PhysicalFunctionRegistryArguments physicalFunctionRegistryArguments)
{
    PRECONDITION(physicalFunctionRegistryArguments.childFunctions.size() == 2, "OverbeforeStboxTspatialPhysicalFunction requires exactly 2 child functions, but got {}", physicalFunctionRegistryArguments.childFunctions.size());
    return OverbeforeStboxTspatialPhysicalFunction(physicalFunctionRegistryArguments.childFunctions[0], physicalFunctionRegistryArguments.childFunctions[1]);
}

...
#include <string>
#include <cstring>
#include <Functions/Meos/ExternBoolGeomContainsPhysicalFunction.hpp>
...
ExternBoolGeomContainsPhysicalFunction::ExternBoolGeomContainsPhysicalFunction(PhysicalFunction lon1Function, PhysicalFunction lat1Function, PhysicalFunction timestamp1Function, PhysicalFunction staticGeometryFunction)
...
ExternBoolGeomContainsPhysicalFunction::ExternBoolGeomContainsPhysicalFunction(PhysicalFunction lon1Function, PhysicalFunction lat1Function, PhysicalFunction timestamp1Function, PhysicalFunction lon2Function, PhysicalFunction lat2Function, PhysicalFunction timestamp2Function)
...
VarVal ExternBoolGeomContainsPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    std::cout << "ExternBoolGeomContainsPhysicalFunction::execute called with " << parameterFunctions.size() << " arguments" << std::endl;
...
VarVal ExternBoolGeomContainsPhysicalFunction::executeTemporal6Param(const std::vector<VarVal>& params) const
{
    ...
    std::cout << "6-param temporal-temporal externBoolGeomContains with coordinate values" << std::endl;
    ...
    const auto result = nautilus::invoke(
        +[](double lon1_val, double lat1_val, uint64_t ts1_val, double lon2_val, double lat2_val, uint64_t ts2_val) -> void {
            try {
                ...
                // Both geometries are temporal points, use temporal-temporal externBoolGeomContains
                std::cout << "Using temporal-temporal externBoolGeomContains (extern bool geom_contains)" << std::endl;
                MEOS::Meos::TemporalGeometry left_temporal(left_geometry_wkt);
                MEOS::Meos::TemporalGeometry right_temporal(right_geometry_wkt);
                void function_result = left_temporal.externBoolGeomContains(right_temporal);
                
                std::cout << "ExternBoolGeomContains result: " << function_result << std::endl;
                return function_result;
            } catch (const std::exception& e) {
                std::cerr << "Error in externBoolGeomContains temporal-temporal: " << e.what() << std::endl;
...
VarVal ExternBoolGeomContainsPhysicalFunction::executeTemporal4Param(const std::vector<VarVal>& params) const
{
    ...
    std::cout << "4-param temporal-static externBoolGeomContains with coordinate and static geometry" << std::endl;
    ...
    const auto result = nautilus::invoke(
        +[](double lon_val, double lat_val, uint64_t timestamp_val, VariableSizedData static_geom_data) -> void {
            try {
                ...
                // Use temporal-static externBoolGeomContains
                std::cout << "Using temporal-static externBoolGeomContains (extern bool geom_contains)" << std::endl;
                MEOS::Meos::TemporalGeometry temporal_geom(temporal_geometry_wkt);
                MEOS::Meos::StaticGeometry static_geom(static_geometry_wkt);
                void function_result = temporal_geom.externBoolGeomContainsStatic(static_geom);
                
                std::cout << "ExternBoolGeomContains result: " << function_result << std::endl;
                return function_result;
            } catch (const std::exception& e) {
                std::cerr << "Error in externBoolGeomContains temporal-static: " << e.what() << std::endl;
                return -1; // Error indicator
            }
        },
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterExternBoolGeomContainsPhysicalFunction(PhysicalFunctionRegistryArguments arguments)
{
    if (arguments.children.size() == 4) {
        return ExternBoolGeomContainsPhysicalFunction(arguments.children[0], arguments.children[1], arguments.children[2], arguments.children[3]);
    } else if (arguments.children.size() == 6) {
        return ExternBoolGeomContainsPhysicalFunction(arguments.children[0], arguments.children[1], arguments.children[2], arguments.children[3], arguments.children[4], arguments.children[5]);
    } else {
        PRECONDITION(false, "ExternBoolGeomContainsPhysicalFunction requires 4 or 6 children, but got {}", arguments.children.size());
    }
}

...
BeforeTspatialStboxPhysicalFunction::BeforeTspatialStboxPhysicalFunction(        PhysicalFunction param0Function,
    ...
    const auto result = nautilus::invoke(
        +[](const char* param0_ptr, uint32_t param0_size, const char* param1_ptr, uint32_t param1_size) -> int32_t {
            try {
                ...
                auto meos_result = MEOS::before_tspatial_stbox(param0_obj, param1_obj);
...
PhysicalFunctionGeneratedRegistrar::RegisterBeforeTspatialStboxPhysicalFunction(PhysicalFunctionRegistryArguments physicalFunctionRegistryArguments)
{
    PRECONDITION(physicalFunctionRegistryArguments.childFunctions.size() == 2, "BeforeTspatialStboxPhysicalFunction requires exactly 2 child functions, but got {}", physicalFunctionRegistryArguments.childFunctions.size());
    return BeforeTspatialStboxPhysicalFunction(physicalFunctionRegistryArguments.childFunctions[0], physicalFunctionRegistryArguments.childFunctions[1]);
}

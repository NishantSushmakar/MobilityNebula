...
#include <string>
#include <cstring>
#include <Functions/Meos/ExternBoolContainsTboxTnumberPhysicalFunction.hpp>
...
ExternBoolContainsTboxTnumberPhysicalFunction::ExternBoolContainsTboxTnumberPhysicalFunction(PhysicalFunction lon1Function, PhysicalFunction lat1Function, PhysicalFunction timestamp1Function, PhysicalFunction staticGeometryFunction)
...
ExternBoolContainsTboxTnumberPhysicalFunction::ExternBoolContainsTboxTnumberPhysicalFunction(PhysicalFunction lon1Function, PhysicalFunction lat1Function, PhysicalFunction timestamp1Function, PhysicalFunction lon2Function, PhysicalFunction lat2Function, PhysicalFunction timestamp2Function)
...
VarVal ExternBoolContainsTboxTnumberPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    std::cout << "ExternBoolContainsTboxTnumberPhysicalFunction::execute called with " << parameterFunctions.size() << " arguments" << std::endl;
...
VarVal ExternBoolContainsTboxTnumberPhysicalFunction::executeTemporal6Param(const std::vector<VarVal>& params) const
{
    ...
    std::cout << "6-param temporal-temporal externBoolContainsTboxTnumber with coordinate values" << std::endl;
    ...
    const auto result = nautilus::invoke(
        +[](double lon1_val, double lat1_val, uint64_t ts1_val, double lon2_val, double lat2_val, uint64_t ts2_val) -> void {
            try {
                ...
                // Both geometries are temporal points, use temporal-temporal externBoolContainsTboxTnumber
                std::cout << "Using temporal-temporal externBoolContainsTboxTnumber (extern bool contains_tbox_tnumber)" << std::endl;
                MEOS::Meos::TemporalGeometry left_temporal(left_geometry_wkt);
                MEOS::Meos::TemporalGeometry right_temporal(right_geometry_wkt);
                void function_result = left_temporal.externBoolContainsTboxTnumber(right_temporal);
                
                std::cout << "ExternBoolContainsTboxTnumber result: " << function_result << std::endl;
                return function_result;
            } catch (const std::exception& e) {
                std::cerr << "Error in externBoolContainsTboxTnumber temporal-temporal: " << e.what() << std::endl;
...
VarVal ExternBoolContainsTboxTnumberPhysicalFunction::executeTemporal4Param(const std::vector<VarVal>& params) const
{
    ...
    std::cout << "4-param temporal-static externBoolContainsTboxTnumber with coordinate and static geometry" << std::endl;
    ...
    const auto result = nautilus::invoke(
        +[](double lon_val, double lat_val, uint64_t timestamp_val, VariableSizedData static_geom_data) -> void {
            try {
                ...
                // Use temporal-static externBoolContainsTboxTnumber
                std::cout << "Using temporal-static externBoolContainsTboxTnumber (extern bool contains_tbox_tnumber)" << std::endl;
                MEOS::Meos::TemporalGeometry temporal_geom(temporal_geometry_wkt);
                MEOS::Meos::StaticGeometry static_geom(static_geometry_wkt);
                void function_result = temporal_geom.externBoolContainsTboxTnumberStatic(static_geom);
                
                std::cout << "ExternBoolContainsTboxTnumber result: " << function_result << std::endl;
                return function_result;
            } catch (const std::exception& e) {
                std::cerr << "Error in externBoolContainsTboxTnumber temporal-static: " << e.what() << std::endl;
                return -1; // Error indicator
            }
        },
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterExternBoolContainsTboxTnumberPhysicalFunction(PhysicalFunctionRegistryArguments arguments)
{
    if (arguments.children.size() == 4) {
        return ExternBoolContainsTboxTnumberPhysicalFunction(arguments.children[0], arguments.children[1], arguments.children[2], arguments.children[3]);
    } else if (arguments.children.size() == 6) {
        return ExternBoolContainsTboxTnumberPhysicalFunction(arguments.children[0], arguments.children[1], arguments.children[2], arguments.children[3], arguments.children[4], arguments.children[5]);
    } else {
        PRECONDITION(false, "ExternBoolContainsTboxTnumberPhysicalFunction requires 4 or 6 children, but got {}", arguments.children.size());
    }
}

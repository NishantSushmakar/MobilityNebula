...
#include <Functions/Meos/EcontainsGeoTgeoPhysicalFunction.hpp>
...
EcontainsGeoTgeoPhysicalFunction::EcontainsGeoTgeoPhysicalFunction(        PhysicalFunction param0Function,
...
VarVal EcontainsGeoTgeoPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](const char* param0_ptr, uint32_t param0_size, const char* param1_ptr, uint32_t param1_size) -> int32_t {
            try {
                ...
                auto meos_result = MEOS::econtains_geo_tgeo(param0_obj, param1_obj);
...
PhysicalFunctionGeneratedRegistrar::RegisterEcontainsGeoTgeoPhysicalFunction(PhysicalFunctionRegistryArguments physicalFunctionRegistryArguments)
{
    PRECONDITION(physicalFunctionRegistryArguments.childFunctions.size() == 2, "EcontainsGeoTgeoPhysicalFunction requires exactly 2 child functions, but got {}", physicalFunctionRegistryArguments.childFunctions.size());
    return EcontainsGeoTgeoPhysicalFunction(physicalFunctionRegistryArguments.childFunctions[0], physicalFunctionRegistryArguments.childFunctions[1]);
}

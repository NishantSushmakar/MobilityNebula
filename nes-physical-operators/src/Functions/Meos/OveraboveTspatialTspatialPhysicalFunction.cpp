...
OveraboveTspatialTspatialPhysicalFunction::OveraboveTspatialTspatialPhysicalFunction(        PhysicalFunction param0Function,
    ...
    const auto result = nautilus::invoke(
        +[](const char* param0_ptr, uint32_t param0_size, const char* param1_ptr, uint32_t param1_size) -> int32_t {
            try {
                ...
                auto meos_result = MEOS::overabove_tspatial_tspatial(param0_obj, param1_obj);
...
PhysicalFunctionGeneratedRegistrar::RegisterOveraboveTspatialTspatialPhysicalFunction(PhysicalFunctionRegistryArguments physicalFunctionRegistryArguments)
{
    PRECONDITION(physicalFunctionRegistryArguments.childFunctions.size() == 2, "OveraboveTspatialTspatialPhysicalFunction requires exactly 2 child functions, but got {}", physicalFunctionRegistryArguments.childFunctions.size());
    return OveraboveTspatialTspatialPhysicalFunction(physicalFunctionRegistryArguments.childFunctions[0], physicalFunctionRegistryArguments.childFunctions[1]);
}

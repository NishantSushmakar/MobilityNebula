...
TcontainsTgeoTgeoPhysicalFunction::TcontainsTgeoTgeoPhysicalFunction(PhysicalFunction lon1Function, PhysicalFunction lat1Function, PhysicalFunction timestamp1Function, PhysicalFunction staticGeometryFunction)
...
TcontainsTgeoTgeoPhysicalFunction::TcontainsTgeoTgeoPhysicalFunction(PhysicalFunction lon1Function, PhysicalFunction lat1Function, PhysicalFunction timestamp1Function, PhysicalFunction lon2Function, PhysicalFunction lat2Function, PhysicalFunction timestamp2Function)
...
{
    std::cout << "TcontainsTgeoTgeoPhysicalFunction::execute called with " << parameterFunctions.size() << " arguments" << std::endl;
...
VarVal TcontainsTgeoTgeoPhysicalFunction::executeTemporal6Param(const std::vector<VarVal>& params) const
{
    ...
    std::cout << "6-param temporal-temporal tcontainsTgeoTgeo with coordinate values" << std::endl;
    ...
    const auto result = nautilus::invoke(
        +[](double lon1_val, double lat1_val, uint64_t ts1_val, double lon2_val, double lat2_val, uint64_t ts2_val) -> extern Temporal * {
            try {
                ...
                // Both geometries are temporal points, use temporal-temporal tcontainsTgeoTgeo
                std::cout << "Using temporal-temporal tcontainsTgeoTgeo (tcontains_tgeo_tgeo)" << std::endl;
                MEOS::Meos::TemporalGeometry left_temporal(left_geometry_wkt);
                MEOS::Meos::TemporalGeometry right_temporal(right_geometry_wkt);
                extern Temporal * function_result = left_temporal.tcontainsTgeoTgeo(right_temporal);
                
                std::cout << "TcontainsTgeoTgeo result: " << function_result << std::endl;
                return function_result;
            } catch (const std::exception& e) {
                std::cerr << "Error in tcontainsTgeoTgeo temporal-temporal: " << e.what() << std::endl;
...
VarVal TcontainsTgeoTgeoPhysicalFunction::executeTemporal4Param(const std::vector<VarVal>& params) const
{
    ...
    std::cout << "4-param temporal-static tcontainsTgeoTgeo with coordinate and static geometry" << std::endl;
    ...
    const auto result = nautilus::invoke(
        +[](double lon_val, double lat_val, uint64_t timestamp_val, VariableSizedData static_geom_data) -> extern Temporal * {
            try {
                ...
                // Use temporal-static tcontainsTgeoTgeo
                std::cout << "Using temporal-static tcontainsTgeoTgeo (tcontains_tgeo_tgeo)" << std::endl;
                MEOS::Meos::TemporalGeometry temporal_geom(temporal_geometry_wkt);
                MEOS::Meos::StaticGeometry static_geom(static_geometry_wkt);
                extern Temporal * function_result = temporal_geom.tcontainsTgeoTgeoStatic(static_geom);
                
                std::cout << "TcontainsTgeoTgeo result: " << function_result << std::endl;
                return function_result;
            } catch (const std::exception& e) {
                std::cerr << "Error in tcontainsTgeoTgeo temporal-static: " << e.what() << std::endl;
                return -1; // Error indicator
            }
        },
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTcontainsTgeoTgeoPhysicalFunction(PhysicalFunctionRegistryArguments arguments)
{
    if (arguments.children.size() == 4) {
        return TcontainsTgeoTgeoPhysicalFunction(arguments.children[0], arguments.children[1], arguments.children[2], arguments.children[3]);
    } else if (arguments.children.size() == 6) {
        return TcontainsTgeoTgeoPhysicalFunction(arguments.children[0], arguments.children[1], arguments.children[2], arguments.children[3], arguments.children[4], arguments.children[5]);
    } else {
        PRECONDITION(false, "TcontainsTgeoTgeoPhysicalFunction requires 4 or 6 children, but got {}", arguments.children.size());
    }
}

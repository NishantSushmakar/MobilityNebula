...
TintersectsGeoTgeoPhysicalFunction::TintersectsGeoTgeoPhysicalFunction(PhysicalFunction lon1Function, PhysicalFunction lat1Function, PhysicalFunction timestamp1Function, PhysicalFunction staticGeometryFunction)
...
TintersectsGeoTgeoPhysicalFunction::TintersectsGeoTgeoPhysicalFunction(PhysicalFunction lon1Function, PhysicalFunction lat1Function, PhysicalFunction timestamp1Function, PhysicalFunction lon2Function, PhysicalFunction lat2Function, PhysicalFunction timestamp2Function)
...
{
    std::cout << "TintersectsGeoTgeoPhysicalFunction::execute called with " << parameterFunctions.size() << " arguments" << std::endl;
...
VarVal TintersectsGeoTgeoPhysicalFunction::executeTemporal6Param(const std::vector<VarVal>& params) const
{
    ...
    std::cout << "6-param temporal-temporal tintersectsGeoTgeo with coordinate values" << std::endl;
    ...
    const auto result = nautilus::invoke(
        +[](double lon1_val, double lat1_val, uint64_t ts1_val, double lon2_val, double lat2_val, uint64_t ts2_val) -> extern Temporal * {
            try {
                ...
                // Both geometries are temporal points, use temporal-temporal tintersectsGeoTgeo
                std::cout << "Using temporal-temporal tintersectsGeoTgeo (tintersects_geo_tgeo)" << std::endl;
                MEOS::Meos::TemporalGeometry left_temporal(left_geometry_wkt);
                MEOS::Meos::TemporalGeometry right_temporal(right_geometry_wkt);
                extern Temporal * function_result = left_temporal.tintersectsGeoTgeo(right_temporal);
                
                std::cout << "TintersectsGeoTgeo result: " << function_result << std::endl;
                return function_result;
            } catch (const std::exception& e) {
                std::cerr << "Error in tintersectsGeoTgeo temporal-temporal: " << e.what() << std::endl;
...
VarVal TintersectsGeoTgeoPhysicalFunction::executeTemporal4Param(const std::vector<VarVal>& params) const
{
    ...
    std::cout << "4-param temporal-static tintersectsGeoTgeo with coordinate and static geometry" << std::endl;
    ...
    const auto result = nautilus::invoke(
        +[](double lon_val, double lat_val, uint64_t timestamp_val, VariableSizedData static_geom_data) -> extern Temporal * {
            try {
                ...
                // Use temporal-static tintersectsGeoTgeo
                std::cout << "Using temporal-static tintersectsGeoTgeo (tintersects_geo_tgeo)" << std::endl;
                MEOS::Meos::TemporalGeometry temporal_geom(temporal_geometry_wkt);
                MEOS::Meos::StaticGeometry static_geom(static_geometry_wkt);
                extern Temporal * function_result = temporal_geom.tintersectsGeoTgeoStatic(static_geom);
                
                std::cout << "TintersectsGeoTgeo result: " << function_result << std::endl;
                return function_result;
            } catch (const std::exception& e) {
                std::cerr << "Error in tintersectsGeoTgeo temporal-static: " << e.what() << std::endl;
                return -1; // Error indicator
            }
        },
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTintersectsGeoTgeoPhysicalFunction(PhysicalFunctionRegistryArguments arguments)
{
    if (arguments.children.size() == 4) {
        return TintersectsGeoTgeoPhysicalFunction(arguments.children[0], arguments.children[1], arguments.children[2], arguments.children[3]);
    } else if (arguments.children.size() == 6) {
        return TintersectsGeoTgeoPhysicalFunction(arguments.children[0], arguments.children[1], arguments.children[2], arguments.children[3], arguments.children[4], arguments.children[5]);
    } else {
        PRECONDITION(false, "TintersectsGeoTgeoPhysicalFunction requires 4 or 6 children, but got {}", arguments.children.size());
    }
}

...
EdwithinTgeoTgeoPhysicalFunction::EdwithinTgeoTgeoPhysicalFunction(        PhysicalFunction param0Function,
...
VarVal EdwithinTgeoTgeoPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto param2 = parameterValues[2].cast<nautilus::val<double>>();

    
    
    // Use nautilus::invoke to call direct MEOS function
    const auto result = nautilus::invoke(
        +[](const char* param0_ptr, uint32_t param0_size, const char* param1_ptr, uint32_t param1_size, double param2) -> int32_t {
            try {
                ...
                auto meos_result = MEOS::edwithin_tgeo_tgeo(param0_obj, param1_obj, param2);
...
PhysicalFunctionGeneratedRegistrar::RegisterEdwithinTgeoTgeoPhysicalFunction(PhysicalFunctionRegistryArguments physicalFunctionRegistryArguments)
{
    PRECONDITION(physicalFunctionRegistryArguments.childFunctions.size() == 3, "EdwithinTgeoTgeoPhysicalFunction requires exactly 3 child functions, but got {}", physicalFunctionRegistryArguments.childFunctions.size());
    return EdwithinTgeoTgeoPhysicalFunction(physicalFunctionRegistryArguments.childFunctions[0], physicalFunctionRegistryArguments.childFunctions[1], physicalFunctionRegistryArguments.childFunctions[2]);
}

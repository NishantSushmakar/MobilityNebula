...
TintersectsTgeoGeoPhysicalFunction::TintersectsTgeoGeoPhysicalFunction(PhysicalFunction lon1Function, PhysicalFunction lat1Function, PhysicalFunction timestamp1Function, PhysicalFunction staticGeometryFunction)
...
TintersectsTgeoGeoPhysicalFunction::TintersectsTgeoGeoPhysicalFunction(PhysicalFunction lon1Function, PhysicalFunction lat1Function, PhysicalFunction timestamp1Function, PhysicalFunction lon2Function, PhysicalFunction lat2Function, PhysicalFunction timestamp2Function)
...
{
    std::cout << "TintersectsTgeoGeoPhysicalFunction::execute called with " << parameterFunctions.size() << " arguments" << std::endl;
...
VarVal TintersectsTgeoGeoPhysicalFunction::executeTemporal6Param(const std::vector<VarVal>& params) const
{
    ...
    std::cout << "6-param temporal-temporal tintersectsTgeoGeo with coordinate values" << std::endl;
    ...
    const auto result = nautilus::invoke(
        +[](double lon1_val, double lat1_val, uint64_t ts1_val, double lon2_val, double lat2_val, uint64_t ts2_val) -> extern Temporal * {
            try {
                ...
                // Both geometries are temporal points, use temporal-temporal tintersectsTgeoGeo
                std::cout << "Using temporal-temporal tintersectsTgeoGeo (tintersects_tgeo_geo)" << std::endl;
                MEOS::Meos::TemporalGeometry left_temporal(left_geometry_wkt);
                MEOS::Meos::TemporalGeometry right_temporal(right_geometry_wkt);
                extern Temporal * function_result = left_temporal.tintersectsTgeoGeo(right_temporal);
                
                std::cout << "TintersectsTgeoGeo result: " << function_result << std::endl;
                return function_result;
            } catch (const std::exception& e) {
                std::cerr << "Error in tintersectsTgeoGeo temporal-temporal: " << e.what() << std::endl;
...
VarVal TintersectsTgeoGeoPhysicalFunction::executeTemporal4Param(const std::vector<VarVal>& params) const
{
    ...
    std::cout << "4-param temporal-static tintersectsTgeoGeo with coordinate and static geometry" << std::endl;
    ...
    const auto result = nautilus::invoke(
        +[](double lon_val, double lat_val, uint64_t timestamp_val, VariableSizedData static_geom_data) -> extern Temporal * {
            try {
                ...
                // Use temporal-static tintersectsTgeoGeo
                std::cout << "Using temporal-static tintersectsTgeoGeo (tintersects_tgeo_geo)" << std::endl;
                MEOS::Meos::TemporalGeometry temporal_geom(temporal_geometry_wkt);
                MEOS::Meos::StaticGeometry static_geom(static_geometry_wkt);
                extern Temporal * function_result = temporal_geom.tintersectsTgeoGeoStatic(static_geom);
                
                std::cout << "TintersectsTgeoGeo result: " << function_result << std::endl;
                return function_result;
            } catch (const std::exception& e) {
                std::cerr << "Error in tintersectsTgeoGeo temporal-static: " << e.what() << std::endl;
                return -1; // Error indicator
            }
        },
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTintersectsTgeoGeoPhysicalFunction(PhysicalFunctionRegistryArguments arguments)
{
    if (arguments.children.size() == 4) {
        return TintersectsTgeoGeoPhysicalFunction(arguments.children[0], arguments.children[1], arguments.children[2], arguments.children[3]);
    } else if (arguments.children.size() == 6) {
        return TintersectsTgeoGeoPhysicalFunction(arguments.children[0], arguments.children[1], arguments.children[2], arguments.children[3], arguments.children[4], arguments.children[5]);
    } else {
        PRECONDITION(false, "TintersectsTgeoGeoPhysicalFunction requires 4 or 6 children, but got {}", arguments.children.size());
    }
}

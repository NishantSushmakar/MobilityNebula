...
#include <string>
#include <cstring>
#include <Functions/Meos/ExternBoolContainsSpanBigintPhysicalFunction.hpp>
...
ExternBoolContainsSpanBigintPhysicalFunction::ExternBoolContainsSpanBigintPhysicalFunction(PhysicalFunction lon1Function, PhysicalFunction lat1Function, PhysicalFunction timestamp1Function, PhysicalFunction staticGeometryFunction)
...
ExternBoolContainsSpanBigintPhysicalFunction::ExternBoolContainsSpanBigintPhysicalFunction(PhysicalFunction lon1Function, PhysicalFunction lat1Function, PhysicalFunction timestamp1Function, PhysicalFunction lon2Function, PhysicalFunction lat2Function, PhysicalFunction timestamp2Function)
...
VarVal ExternBoolContainsSpanBigintPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    std::cout << "ExternBoolContainsSpanBigintPhysicalFunction::execute called with " << parameterFunctions.size() << " arguments" << std::endl;
...
VarVal ExternBoolContainsSpanBigintPhysicalFunction::executeTemporal6Param(const std::vector<VarVal>& params) const
{
    ...
    std::cout << "6-param temporal-temporal externBoolContainsSpanBigint with coordinate values" << std::endl;
    ...
    const auto result = nautilus::invoke(
        +[](double lon1_val, double lat1_val, uint64_t ts1_val, double lon2_val, double lat2_val, uint64_t ts2_val) -> void {
            try {
                ...
                // Both geometries are temporal points, use temporal-temporal externBoolContainsSpanBigint
                std::cout << "Using temporal-temporal externBoolContainsSpanBigint (extern bool contains_span_bigint)" << std::endl;
                MEOS::Meos::TemporalGeometry left_temporal(left_geometry_wkt);
                MEOS::Meos::TemporalGeometry right_temporal(right_geometry_wkt);
                void function_result = left_temporal.externBoolContainsSpanBigint(right_temporal);
                
                std::cout << "ExternBoolContainsSpanBigint result: " << function_result << std::endl;
                return function_result;
            } catch (const std::exception& e) {
                std::cerr << "Error in externBoolContainsSpanBigint temporal-temporal: " << e.what() << std::endl;
...
VarVal ExternBoolContainsSpanBigintPhysicalFunction::executeTemporal4Param(const std::vector<VarVal>& params) const
{
    ...
    std::cout << "4-param temporal-static externBoolContainsSpanBigint with coordinate and static geometry" << std::endl;
    ...
    const auto result = nautilus::invoke(
        +[](double lon_val, double lat_val, uint64_t timestamp_val, VariableSizedData static_geom_data) -> void {
            try {
                ...
                // Use temporal-static externBoolContainsSpanBigint
                std::cout << "Using temporal-static externBoolContainsSpanBigint (extern bool contains_span_bigint)" << std::endl;
                MEOS::Meos::TemporalGeometry temporal_geom(temporal_geometry_wkt);
                MEOS::Meos::StaticGeometry static_geom(static_geometry_wkt);
                void function_result = temporal_geom.externBoolContainsSpanBigintStatic(static_geom);
                
                std::cout << "ExternBoolContainsSpanBigint result: " << function_result << std::endl;
                return function_result;
            } catch (const std::exception& e) {
                std::cerr << "Error in externBoolContainsSpanBigint temporal-static: " << e.what() << std::endl;
                return -1; // Error indicator
            }
        },
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterExternBoolContainsSpanBigintPhysicalFunction(PhysicalFunctionRegistryArguments arguments)
{
    if (arguments.children.size() == 4) {
        return ExternBoolContainsSpanBigintPhysicalFunction(arguments.children[0], arguments.children[1], arguments.children[2], arguments.children[3]);
    } else if (arguments.children.size() == 6) {
        return ExternBoolContainsSpanBigintPhysicalFunction(arguments.children[0], arguments.children[1], arguments.children[2], arguments.children[3], arguments.children[4], arguments.children[5]);
    } else {
        PRECONDITION(false, "ExternBoolContainsSpanBigintPhysicalFunction requires 4 or 6 children, but got {}", arguments.children.size());
    }
}
